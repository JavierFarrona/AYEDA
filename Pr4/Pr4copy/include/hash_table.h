...
// Función hash para Persona
template <>
class Module<Persona> {
  public:
    Module(unsigned tableSize) : tableSize_(tableSize) {}
    unsigned operator()(const Persona& persona) const {
        // Usa el NIF y la letra como base para calcular el hash
        unsigned hash = static_cast<unsigned>(persona.getNif());
        hash += static_cast<unsigned>(persona.getLetra()); // Suma el valor ASCII de la letra
        return hash % tableSize_;
    }
  private:
    unsigned tableSize_;
};
...
class Module<Alumno> {
  public:
    Module(unsigned tableSize) : tableSize_(tableSize) {}
    unsigned operator()(const Alumno& alumno) const {
        // Usa el atributo key_ para calcular el hash
        return alumno.getKey() % tableSize_;
    }
  private:
    unsigned tableSize_;
};
